...
pub struct ClaimAndSend<'info> {
    ...
    #[account(mut, constraint = reward_a_destination.mint == reward_a_mint.key())]
    ...
    #[account(mut, constraint = reward_b_destination.mint == reward_b_mint.key())]
...
pub fn handler(ctx: Context<ClaimAndSend>) -> Result<()> {
